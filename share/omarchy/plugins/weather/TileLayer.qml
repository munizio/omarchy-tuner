...
Item {
  ...
  // True once every requested tile is Ready or Error. The radar loop waits
  // on this so it does not fade in an empty layer over the previous frame.
  ...
  function recomputeReady() {
    ...
    for (var i = 0; i < n; i++) {
      ...
      if (!item.source || item.source === "") continue
      if (item.status === Image.Loading || item.status === Image.Null) {
        tilesReady = false
        return
      }
    }
